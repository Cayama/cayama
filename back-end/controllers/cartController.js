...
const updateShoppingCart = rescue(async (req, res, next) => {
  ...
  await cartService.updateShoppingCart(userId, totalPrice, purchases);

  return res.status(201).json({ response: 'Alteração do carrinho realizada com sucesso' });
});
...
const deleteShoppingCart = rescue(async (req, res, next) => {
  ...
  return res.status(201).json({ response: 'Carrinho deletado com sucesso' });
});
